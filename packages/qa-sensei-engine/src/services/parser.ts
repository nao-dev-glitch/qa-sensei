import { TscAnalysisResult, TscError, CodeFile } from '../types/index.js';

export class TscLogParser {
  /**
   * tsc --pretty false の出力を構造化データへ変換
   */
  static parse(rawLog: string): TscAnalysisResult {
    const regex = /^(.*?)\((\d+),(\d+)\):\s+error\s+(TS\d+):\s+(.*)$/gm;
    const errorsByFile: Record<string, TscError[]> = {};
    ...
    while ((match = regex.exec(rawLog)) !== null) {
      ...
      const errorObj: TscError = { filePath, line, column, code, message: message.trim() };
   ...
   */
  static extractRelevantFiles(allFiles: CodeFile[], analysis: TscAnalysisResult): CodeFile[] {
