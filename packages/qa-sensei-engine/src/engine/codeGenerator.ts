import { CodeFile, EscalationReport, AttemptLog } from '../types/index.js';
...
export class SelfHealingEngine {
   ...
   */
  private aiCaller: (prompt: string) => Promise<CodeFile[]>;

  constructor(aiCaller: (prompt: string) => Promise<CodeFile[]>) {
   ...
   */
  async execute(initialPrompt: string): Promise<{ success: boolean; files?: CodeFile[]; escalation?: EscalationReport }> {
  ...
  private buildRepairPrompt(errorSummary: string, targetFiles: CodeFile[], history: AttemptLog[]): string {
